            ...
            if ($result->num_rows > 0) {
                ...
                <table class="table table-striped table-bordered cart-table">
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $i++;
                        $pid=$row['product_id'];
                        $selStock="SELECT SUM(stock_qty) as sum from tbl_stock where product_id ='$pid'";
                        $resStock=$Con->query($selStock);
                        $dataStock=$resStock->fetch_assoc();
                        $totalStock=$dataStock['sum'];
                        $selCart="SELECT SUM(cart_qty) as sum from tbl_cart where cart_status>=1 AND cart_status<6 And  product_id ='$pid'";
                        ...
                        <tr>
                            ...
                            <td>
                                <input required type="number" class="form-control" min='1' max='<?php echo $remStock ?>' name="txt_qty" id="txt_qty" value="<?php echo $row['cart_qty']; ?>" onChange="TotalPrice(this.value,'<?php echo $row['cart_id'] ?>')"/>
                            </td>
