        ...
        <div class="row">
            ...
            <div class="col-md-8">
                <div class="product-details">
                    ...
                    <a href="#" onclick="addCart(<?php echo $row['product_id'] ?>)" class="btn-addwish-b2 dis-block pos-relative js-addwish-b2">
                                <i style="font-size:28px;" class="zmdi zmdi-shopping-cart"></i>
								</a>
                </div>
...
<div class="rating-section mt-5">
        ...
        <?php
        $selRating = "SELECT r.rating_value, r.rating_content, r.rating_datetime, u.user_name 
                      FROM tbl_rating r
                      INNER JOIN tbl_user u ON r.user_id = u.user_id 
                      WHERE r.product_id = '$id'
                      ORDER BY r.rating_datetime DESC";
        $ratingResult = $Con->query($selRating);
        
        if ($ratingResult->num_rows > 0) {
            while ($rating = $ratingResult->fetch_assoc()) {
        ?>
        <div class="rating-card">
            <h5 class="mb-1"><?php echo $rating['user_name']; ?></h5>
            <div class="rating-value">
                <?php echo str_repeat('★', $rating['rating_value']); ?>
            </div>
            <p class="mb-0"><?php echo $rating['rating_content']; ?></p>
            <small class="text-muted"><?php echo $rating['rating_datetime']; ?></small>
        </div>
        <?php
            }
        } else {
            echo "<p class='text-center text-muted'>No ratings yet for this product.</p>";
        }
        ?>
    </div>
    <script>
